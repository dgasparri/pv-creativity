...
const std::vector<geometry::vertex *> panel_io::load_vertices(const std::string filename)
{
	...
	do {
		...
		if (assigned < 9 || assigned == EOF)
			break;
		v.emplace_back(new geometry::vertex(v1x, v1y, v1z));
		v.emplace_back(new geometry::vertex(v2x, v2y, v2z));
		v.emplace_back(new geometry::vertex(v3x, v3y, v3z));
...
const std::vector<geometry::vertex *> panel_io::test_vertices_2()
{
	...
	double L = 3;
	double V = 4;
	vertices.emplace_back(new geometry::vertex(-V, -L, 0.0));
	vertices.emplace_back(new geometry::vertex(V, -L, 0.0));
	vertices.emplace_back(new geometry::vertex(0.0, 0.0, V));
