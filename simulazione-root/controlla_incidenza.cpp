...
double compute_cos_theta_part(double L_rad, double beta_rad, double Z_S_rad, double delta, double h);
...
void compute_absorbed_radiation_S(
	...
	double cos_theta1 = compute_cos_theta_part(L_rad, beta_rad, 
		Z_S_rad, delta_rad, h_rad);

	std::cout<<"cos theta part 1: "<<cos_theta1<<" theta "<<acos(cos_theta1)<<std::endl;
...
//Eq. 4n
//TODO
double compute_cos_theta_part(double L_rad, double beta_rad, double Z_S_rad, double delta_rad, double h) {
	return sin(L_rad) * sin(delta_rad) * cos(beta_rad)
		+ cos(L_rad) * cos(delta_rad) * cos(h) * cos(beta_rad);
	double a =  sin(L_rad) * cos(delta_rad) * cos(h) * sin(beta_rad) * cos(Z_S_rad)
		+ cos(delta_rad) * sin(h) * sin(beta_rad) * sin(Z_S_rad);
}
